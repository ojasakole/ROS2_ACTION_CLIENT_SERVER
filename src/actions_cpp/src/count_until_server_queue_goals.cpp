...
public:
    CountUntilServerNode() : Node("count_until_server")
    {
        ...
        count_until_server = rclcpp_action::create_server<CountUntil>(
            this,
            "count_until",
...
private:
    ...
    void execute_goal(const std::shared_ptr<CountUntilGoalHandle> goal_handle)
    {
        ...
        // Add short sleep so that all feedback is sent and received
        ...
        // Set final state and return that 
        result->reached_number = counter;
        goal_handle->succeed(result);
    }

    rclcpp_action::Server<CountUntil>::SharedPtr count_until_server;
